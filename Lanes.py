...
class Lanes:
    """
    Copied from Paul Hearty's Lanes class: https://github.com/PaulHeraty/AdvancedLanefinding/blob/master/Lanes.py
    """
    ...
    def check_lane(self, new_roc, prev_roc, new_x, prev_x):
        ...
        if new_roc < 587:
            if self.debug_mode:
                print("Lane Discarded! : Frame {}, Lane RoC out of bounds {} (587)".format(
                    self.frame_number - 1, new_roc))
            return False
    ...
    def fit_lanes(self):
        ...
        if self.frame_number == 1 or \
                self.check_lane(left_curverad, self.left_curverad, self.left_x, self.last_left_x):
            ...
            if self.debug_mode:
                print("Frame : {} Left Lane found! RoC : {} Prev X : {} Curr X : {}".format(
                    self.frame_number - 1, left_curverad, self.last_left_x, self.left_x))
        ...
        # Only use this new lane fit if it's close to the previous one (for smoothing)
        if self.frame_number == 1 or \
                self.check_lane(right_curverad, self.right_curverad, self.right_x, self.last_right_x):
            ...
            if self.debug_mode:
                print("Frame : {} Right Lane found! RoC : {} Prev X : {} Curr X : {}".format(
                    self.frame_number - 1, right_curverad, self.last_right_x, self.right_x))
